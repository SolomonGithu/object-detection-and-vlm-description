...
function initSocketIO() {
  ...
  // Puts image and it's vlm description on UI
    socket.on('vlm_response', (data) => {
        const vlmDisplay = document.getElementById('vlm-display');
        const vlmPlaceholder = document.getElementById('vlm-placeholder');
        
        console.log("searching for elements...", { vlmDisplay, vlmPlaceholder });

        if (vlmDisplay && vlmPlaceholder) {
            vlmPlaceholder.style.display = 'none';
            vlmDisplay.style.display = 'flex';
            document.getElementById('vlm-text').textContent = data.text;
            document.getElementById('vlm-image').src = `img/vlm_temp.jpg?t=${Date.now()}`;
        } else {
            console.error("vlm ui elements not found in index.html");
        }
    });
...
// Updated for VLM result
function updateFeedback(detection) {
    const vlmPlaceholder = document.getElementById('vlm-placeholder');
    const vlmDisplay = document.getElementById('vlm-display');

    // do not let the feedback overwrite VLM feedback
    if (vlmDisplay && vlmDisplay.style.display === 'flex') {
        return; 
    }
